...
internal static class Strings
{
    ...
    private static readonly string[] Keys =
    {
        ...
        Lore, Prestige, Refund, Options, Report, Continue, PressKey, Hint, ResetAll, Conflict, ToggleSpells };
    ...
    private static readonly Dictionary<string, string[]> Table = new()
    {
        ["en"] = new[] { "Keyboard & Mouse", "Controller", "In a run", "Menus", "Move up", "Move down", "Move left", "Move right", "Move (stick)",
            ...
            "Esc — cancel. Menu navigation (arrows, Space, Esc) always stays the same.", "Reset all", "Orange — the key is also used by another action here.", "Show / hide spells" },
        ["ru"] = new[] { "Клавиатура и мышь", "Контроллер", "В забеге", "Меню", "Идти вверх", "Идти вниз", "Идти влево", "Идти вправо", "Движение (стик)",
            ...
            "Esc — отмена. Навигация по меню (стрелки, пробел, Esc) не меняется.", "Сбросить всё", "Оранжевым — клавиша занята ещё одним действием в этом разделе.", "Показать / скрыть заклинания" },
        ["uk"] = new[] { "Клавіатура і миша", "Контролер", "У забігу", "Меню", "Йти вгору", "Йти вниз", "Йти ліворуч", "Йти праворуч", "Рух (стік)",
            ...
            "Esc — скасувати. Навігація меню (стрілки, пробіл, Esc) не змінюється.", "Скинути все", "Помаранчевим — клавіша зайнята ще однією дією в цьому розділі.", "Показати / сховати заклинання" },
        ["de"] = new[] { "Tastatur & Maus", "Controller", "Im Durchlauf", "Menüs", "Nach oben", "Nach unten", "Nach links", "Nach rechts", "Bewegen (Stick)",
            ...
            "Esc — abbrechen. Die Menüsteuerung (Pfeile, Leertaste, Esc) bleibt immer gleich.", "Alles zurücksetzen", "Orange — die Taste ist hier auch einer anderen Aktion zugewiesen.", "Zauber ein-/ausblenden" },
        ["fr"] = new[] { "Clavier et souris", "Manette", "En partie", "Menus", "Aller en haut", "Aller en bas", "Aller à gauche", "Aller à droite", "Déplacement (stick)",
            ...
            "Échap — annuler. La navigation des menus (flèches, Espace, Échap) ne change pas.", "Tout réinitialiser", "En orange — la touche est aussi utilisée par une autre action ici.", "Afficher / masquer les sorts" },
        ["it"] = new[] { "Tastiera e mouse", "Controller", "In partita", "Menu", "Muovi su", "Muovi giù", "Muovi a sinistra", "Muovi a destra", "Movimento (stick)",
            ...
            "Esc — annulla. La navigazione dei menu (frecce, Spazio, Esc) resta invariata.", "Ripristina tutto", "In arancione — il tasto è usato anche da un'altra azione qui.", "Mostra / nascondi incantesimi" },
        ["nl"] = new[] { "Toetsenbord & muis", "Controller", "In een run", "Menu's", "Omhoog", "Omlaag", "Naar links", "Naar rechts", "Bewegen (stick)",
            ...
            "Esc — annuleren. Menunavigatie (pijlen, spatie, Esc) blijft altijd hetzelfde.", "Alles resetten", "Oranje — de toets wordt hier ook door een andere actie gebruikt.", "Spreuken tonen / verbergen" },
        ["pl"] = new[] { "Klawiatura i mysz", "Kontroler", "W wyprawie", "Menu", "Ruch w górę", "Ruch w dół", "Ruch w lewo", "Ruch w prawo", "Ruch (gałka)",
            ...
            "Esc — anuluj. Nawigacja w menu (strzałki, spacja, Esc) pozostaje bez zmian.", "Resetuj wszystko", "Na pomarańczowo — klawisz jest tu używany także przez inną akcję.", "Pokaż / ukryj zaklęcia" },
        ["pt-br"] = new[] { "Teclado e mouse", "Controle", "Na partida", "Menus", "Mover para cima", "Mover para baixo", "Mover para a esquerda", "Mover para a direita", "Mover (analógico)",
            ...
            "Esc — cancelar. A navegação dos menus (setas, Espaço, Esc) não muda.", "Redefinir tudo", "Em laranja — a tecla também é usada por outra ação aqui.", "Mostrar / ocultar feitiços" },
        ["pt"] = new[] { "Teclado e rato", "Comando", "Na partida", "Menus", "Mover para cima", "Mover para baixo", "Mover para a esquerda", "Mover para a direita", "Mover (analógico)",
            ...
            "Esc — cancelar. A navegação dos menus (setas, Espaço, Esc) não muda.", "Repor tudo", "A laranja — a tecla também é usada por outra ação aqui.", "Mostrar / ocultar feitiços" },
        ["es"] = new[] { "Teclado y ratón", "Mando", "En la partida", "Menús", "Mover arriba", "Mover abajo", "Mover a la izquierda", "Mover a la derecha", "Mover (stick)",
            ...
            "Esc — cancelar. La navegación de menús (flechas, Espacio, Esc) no cambia.", "Restablecer todo", "En naranja — la tecla también la usa otra acción aquí.", "Mostrar / ocultar hechizos" },
        ["ja"] = new[] { "キーボード＆マウス", "コントローラー", "ラン中", "メニュー", "上へ移動", "下へ移動", "左へ移動", "右へ移動", "移動（スティック）",
            ...
            "Esc — キャンセル。メニュー操作（矢印、スペース、Esc）は変わりません。", "すべてリセット", "オレンジ色 — このキーはここで別の操作にも割り当てられています。", "呪文の表示 / 非表示" },
        ["ko"] = new[] { "키보드 및 마우스", "컨트롤러", "런 중", "메뉴", "위로 이동", "아래로 이동", "왼쪽으로 이동", "오른쪽으로 이동", "이동 (스틱)",
            ...
            "Esc — 취소. 메뉴 조작(화살표, 스페이스, Esc)은 바뀌지 않습니다.", "모두 초기화", "주황색 — 이 키는 여기서 다른 동작에도 사용됩니다.", "주문 표시 / 숨기기" },
        ["zh-hans"] = new[] { "键盘和鼠标", "手柄", "局内", "菜单", "向上移动", "向下移动", "向左移动", "向右移动", "移动（摇杆）",
            ...
            "Esc — 取消。菜单操作（方向键、空格、Esc）保持不变。", "全部重置", "橙色 — 该按键在此处也被其他操作使用。", "显示 / 隐藏法术" },
        ["zh-hant"] = new[] { "鍵盤和滑鼠", "手把", "局內", "選單", "向上移動", "向下移動", "向左移動", "向右移動", "移動（搖桿）",
            ...
            "Esc — 取消。選單操作（方向鍵、空白鍵、Esc）保持不變。", "全部重設", "橘色 — 該按鍵在此處也被其他操作使用。", "顯示 / 隱藏法術" },
        ["th"] = new[] { "คีย์บอร์ดและเมาส์", "คอนโทรลเลอร์", "ในรอบ", "เมนู", "เดินขึ้น", "เดินลง", "เดินซ้าย", "เดินขวา", "เดิน (สติ๊ก)",
            ...
            "Esc — ยกเลิก การควบคุมเมนู (ลูกศร, Space, Esc) จะไม่เปลี่ยน", "รีเซ็ตทั้งหมด", "สีส้ม — ปุ่มนี้ถูกใช้กับการกระทำอื่นในหมวดนี้ด้วย", "แสดง / ซ่อนเวทมนตร์" },
    };

    private static readonly Localizer Text = new(Keys, Table, () => KeybindsMod.Log);

    public static string Get(string key) => Text.Get(key);

    // Текст, который сам обновится при смене языка
    public static void Bind(TMP_Text text, Func<string> value) => Text.Bind(text, value);
